...
#define OV02B1B_REG_VTOTAL		CCI_REG16(0x14)
#define OV02B1B_VTOTAL_MAX		0x7fff
			   ...
			   struct v4l2_subdev_format *fmt)
{
	...
	if (fmt->which == V4L2_SUBDEV_FORMAT_TRY) {
		*v4l2_subdev_state_get_format(sd_state, 0) = fmt->format;
		...
		vblank_def = mode->vblank - mode->height;
		__v4l2_ctrl_modify_range(ov02b1b->vblank, vblank_def,
					 OV02B1B_VTOTAL_MAX - mode->height, 1,
					 vblank_def);
		__v4l2_ctrl_s_ctrl(ov02b1b->vblank, vblank_def);

		hblank_def = mode->hblank - mode->width;
...
static int ov02b1b_set_vblank(struct ov02b1b *ov02b1b, int val)
{
	...
	ret = cci_write(ov02b1b->regmap, OV02B1B_REG_VTOTAL, val, NULL);
	if (ret)
		return ret;
...
static int ov02b1b_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	case V4L2_CID_VBLANK:
		ret = ov02b1b_set_vblank(ov02b1b, ov02b1b->cur_mode->height + ctrl->val);
		break;
...
static int ov02b1b_initialize_controls(struct ov02b1b *ov02b1b)
{
	...
	ov02b1b->vblank = v4l2_ctrl_new_std(handler, &ov02b1b_ctrl_ops, V4L2_CID_VBLANK, vblank_def,
			  		    OV02B1B_VTOTAL_MAX - mode->height, 1, vblank_def);

	exposure_max = mode->vblank - OV02B1B_EXPOSURE_MAX_MARGIN;
